...
Gauss_Seidel<T, U, V>::T& operator()()
{
  ...
  U current_guess_vector(m_a_matrix.get_number_of_rows());
  for ( long i = 0; i < m_a_matrix.get_number_of_rows(); i++)
  {
    current_guess_vector[i] = 0;
  }
  U next_guess_vector(current_guess_vector);
  ...
  do
  {
    ...
    L_Two_Norm<U> current_norm_value(next_guess_vector);
    L_Two_Norm<U> last_norm_value(current_guess_vector);
